...
def main():
    ...
    text=open(args.text_file,encoding='utf-8').read();segs=split_text(text,args.max_chars);p=load_profile(args.profile);v=p.get('voice','x6_lingyuyan_pro');sp=p.get('speed',50);vol=p.get('volume',52);pi=p.get('pitch',50);url=p.get('ws_url','wss://cbm01.cn-huabei-1.xf-yun.com/v1/private/mcd9m97e6')
    with tempfile.TemporaryDirectory() as d:
        parts=[]
        for i,s in enumerate(segs):path=os.path.join(d,f'{i:03d}.mp3');run_once(url,path,v,s,sp,vol,pi);parts.append(path)
